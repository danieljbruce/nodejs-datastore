  ...
  (clientOptions: DatastoreOptions) => {
    describe('Request', () => {
      ...
      beforeEach(() => {
        ...
        v1FakeClientOverride = null;
        request = new Request();
        request.datastore = Object.assign(clientOptions, {
      ...
      describe('createReadStream', () => {
        ...
        describe('success', () => {
            ...
            found: [
              {
                entity: {
                  ...
                  properties: {
                    title: {
                      stringValue:
                        'How to make the perfect pizza in your grill',
                    },
                    tags: {
                      arrayValue: {
                        values: [
          ...
          const expectedResult = entity.formatArray(
            apiResponse.found as Any
          )[0];
          ...
          it('should format results', done => {
            ...
            request
              ...
              .on('end', () => {
                assert.deepStrictEqual(
                  entities,
                  apiResponse.batch.entityResults
                );
                done();
              });
          });
            ...
            it('should pass `wrapNumbers` to runQueryStream as undefined by default', done => {
              request.runQuery(query, (err: Error) => {
                ...
                const runQueryOptions =
                  request.runQueryStream.getCall(0).args[1];
        ...
        it('transaction should rollback if error on transaction run!', done => {
          sandbox
            ...
            .callsFake((gaxOption, callback?: Function) => {
              callback =
                typeof gaxOption === 'function' ? gaxOption : callback!;
